...
(
 ...
 (
    SELECT MK_ACCOUNT_VNT_ID,
           TO_DATE(DATEADD(DAY,-2,LAST_DAY(RESULT_DATE,'week'))) AS TIME_BCKT_DATE,
 ...
 (
    SELECT RIGHT(ACCOUNT_1,18) AS CUSTOMER_ID,
           TO_DATE(DATEADD(DAY,-2,LAST_DAY(KPI_DATE,'week'))) AS TIME_BCKT_DATE,
...
GROUP BY MK_ACCOUNT_VNT_ID,PRODUCT_NAME,TIME_BCKT_DATE
...
SELECT 
    CUSTOMER_ID,
    PRODUCT_NAME,
    MAX(TO_DATE(DATE_OF_ACTIVITY)) AS TIME_BCKT_DATE,
    'M103' AS METRIC_ID,
    'HCP_Last_Call_Activity_Date' AS METRIC_NAME,
    1 AS METRIC_VALUE
FROM call_activity_fact
...
FROM shipment
GROUP BY CUSTOMER_ID,PRODUCT_NAME,TIME_BCKT_DATE
