...
affiliation AS (SELECT MK_ACCOUNT_VNT_ID,
					   MK_PARENT_ACCOUNT_VNT_ID,
					   MODIFIED_DATE__V,
					   RECORD_STATE__V,
					   INGESTION_DATE,
					   HIERARCHY_TYPE__V,
					   UPPER(AFFILIATION_TYPE) AS AFFILIATION_TYPE,
					   UPPER(AFFILIATION_NAME) AS AFFILIATION_NAME 
                FROM __SNOWFLAKE_NGDW_DB__.INTEGRATED.VNT_PARENTHCO_SCD1
				UNPIVOT(
					AFFILIATION_TYPE 
					FOR AFFILIATION_NAME IN (AD_PRIMARY_AFFILIATION__C,AD_INFUSION_TREATMENT_LOCATION__C,AD_LP_SCAN_LOCATION__C,
					                                AD_MRI_SCAN_LOCATION__C,AD_NEUROCOGNITIVE_TESTING_LOCATION__C,AD_PET_SCAN_LOCATION__C)
						)
                    WHERE RECORD_STATE__V = 'VALID'
					...
					AND AFFILIATION_TYPE IS NOT NULL AND AFFILIATION_TYPE <> 'N'), 						
...
vnt_parenthco AS (
			...
			FROM affiliation
			WHERE UPPER(affiliation.AFFILIATION_NAME) = 'AD_PRIMARY_AFFILIATION__C'
			      AND UPPER(affiliation.AFFILIATION_TYPE) = 'Y'
...
SELECT
	...
	CASE WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_PRIMARY_AFFILIATION__C' 
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'Y' THEN 'PRIMARY AD AFFILIATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_INFUSION_TREATMENT_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'PRIMARY__C' THEN 'PRIMARY AD INFUSION TREATMENT LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_INFUSION_TREATMENT_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'SECONDARY__C' THEN 'SECONDARY AD INFUSION TREATMENT LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_PET_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'PRIMARY__C' THEN 'PRIMARY AD PET SCAN LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_PET_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'SECONDARY__C' THEN 'SECONDARY AD PET SCAN LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_NEUROCOGNITIVE_TESTING_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'PRIMARY__C' THEN 'PRIMARY AD NEUROCOGNITIVE TESTING LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_NEUROCOGNITIVE_TESTING_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'SECONDARY__C' THEN 'SECONDARY AD NEUROCOGNITIVE TESTING LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_MRI_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'PRIMARY__C' THEN 'PRIMARY AD MRI SCAN LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_MRI_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'SECONDARY__C' THEN 'SECONDARY AD MRI SCAN LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_LP_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'PRIMARY__C' THEN 'PRIMARY AD LP SCAN LOCATION'
		 WHEN UPPER(affiliation.AFFILIATION_NAME) = 'AD_LP_SCAN_LOCATION__C'
			  AND UPPER(affiliation.AFFILIATION_TYPE) = 'SECONDARY__C' THEN 'SECONDARY AD LP SCAN LOCATION'
		 ELSE NULL END AS AFFILIATION_TYPE,
...
QUALIFY ROW_NUMBER() OVER (PARTITION BY PARENT_ID,CHILD_ID,affiliation.AFFILIATION_NAME ORDER BY affiliation.MODIFIED_DATE__V DESC) = 1  
);
