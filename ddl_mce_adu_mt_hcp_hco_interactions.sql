...
/** Included in CTE to improve performance**/  
    ...
    (
		...
		WHERE UPPER(ACCOUNT_CLASS) IN ('HCP','HCO')
    )
...
SELECT
	...
	marketing_activity_fact.APPROVEDFORDISTRIBUTION_DATE AS APPROVED_FOR_DISTRIBUTION_DATE,
...
WHERE 
	UPPER(marketing_activity_fact.SOURCE_CODE) NOT IN ('KNIPPER','SFDC_US')
...
SELECT
	...
	NULL AS APPROVED_FOR_DISTRIBUTION_DATE,
	ad_dimension.ACCOUNT_STATUS_NAME AS IS_ACTIVE_FLAG
